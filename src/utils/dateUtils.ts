...
/**
 * 将时间描述转换为具体的日期或特殊的重复任务标记
 * @param timelineDesc 时间描述，例如"本周"、"下个月"、"周一/周三/周五"、"每天"
 * @returns ISO 格式的日期字符串或特殊的重复任务标记
 */
export function convertTimelineToDate(timelineDesc: string): string | undefined {
  ...
  if (lowerDesc.includes('每天') || lowerDesc.includes('daily') || lowerDesc === 'everyday') {
    // 返回特殊标记表示每天重复的任务
    console.log(`- Matched "每天": Returning RECURRING_DAILY marker`);
    return RECURRING_TASK_TYPES.DAILY;
  }
  
  if (lowerDesc.includes('持续') || lowerDesc.includes('ongoing')) {
    // 返回特殊标记表示持续进行的任务
    console.log(`- Matched "持续进行": Returning ONGOING marker`);
    return RECURRING_TASK_TYPES.ONGOING;
  }
  ...
  if (lowerDesc.includes('每周')) {
    // 返回特殊标记表示每周重复的任务
    console.log(`- Matched "每周": Returning RECURRING_WEEKLY marker`);
    return RECURRING_TASK_TYPES.WEEKLY;
  }

  // 处理"每月"的情况
  if (lowerDesc.includes('每月')) {
    // 返回特殊标记表示每月重复的任务
    console.log(`- Matched "每月": Returning RECURRING_MONTHLY marker`);
    return RECURRING_TASK_TYPES.MONTHLY;
  }
...
/**
 * 检查日期字符串是否表示重复或持续性任务
 * @param dateString 日期字符串或特殊标记
 * @returns 是否为重复或持续性任务
 */
export function isRecurringTask(dateString: string | undefined): boolean {
  if (!dateString) return false;
  
  return Object.values(RECURRING_TASK_TYPES).includes(dateString as any);
}
 ...
 * @returns 重复任务的类型描述
 */
export function getRecurringTaskTypeDisplay(dateString: string | undefined): string {
  if (!dateString) return '';
  
  switch (dateString) {
    case RECURRING_TASK_TYPES.DAILY:
      return '每天';
    case RECURRING_TASK_TYPES.WEEKLY:
      return '每周';
    case RECURRING_TASK_TYPES.MONTHLY:
      return '每月';
    case RECURRING_TASK_TYPES.ONGOING:
      return '持续进行';
    default:
      return '';
  }
 ...
 * @param dateString ISO 格式的日期字符串或特殊标记
 * @returns 格式化后的日期字符串
 */
export function formatDateDisplay(dateString: string | undefined): string {
  ...
  // 检查是否为重复任务
  if (isRecurringTask(dateString)) {
    return getRecurringTaskTypeDisplay(dateString);
  }
